...
export default function Simulator() {
  ...
  const onClickObstacle = () => {
    ...
    // If already added at postition, return
    for (const o of obstacles) {
      if (o.x === obXInput && o.y === obYInput) return;
    }
    ...
    for (let i = 0; i < 20; i++) {
      const cells = [
        ...
        <td key={i} className="w-5 h-5 md:w-8 md:h-8">
          <span className="text-slate-400 font-bold text-[0.6rem] md:text-base ">
            {19 - i}
          </span>
        </td>,
      ];
      ...
      for (let j = 0; j < 20; j++) {
        ...
        if (foundOb) {
          ...
          if (foundRobotCell.d !== null) {
            cells.push(
              <td
                className={`border-none rounded-b-md w-5 h-5 md:w-8 md:h-8 ${
                  foundRobotCell.s != -1 ? "bg-red-500" : "bg-indigo-500"
                }`}
              />
            );
          } else {
            cells.push(
              <td className="bg-yellow-400 border-none border w-5 h-5 md:w-8 md:h-8" />
            );
          }
        } else {
          cells.push(
            <td className="border-indigo-600 border-2 rounded-md w-5 h-5 md:w-8 md:h-8 bg-slate-900"/>
          );
        }
      }
    ...
    <div className="flex flex-row items-center justify-center gap-x-16">
      <div className="flex flex-col items-center justify-center">
        <div className="flex flex-col items-center text-center">
          <h2 className="text-4xl font-bold text-slate-400 pb-5">Algorithm Simulator</h2>
        </div>

        <div className="flex flex-col items-center text-center">
          <h2 className="text-l text-slate-400 max-w-md pb-5">To begin, adjust the robot's intial position or add obstacles to the grid using the controls below. Once done, click submit.</h2>
        </div>
        ...
        {/* <div className="btn-group btn-group-horizontal pb-10"> */}
        <div className="flex flex-row gap-x-4 pb-10">
          <button className="bg-red-500 hover:bg-red-400 text-white font-bold py-2 px-4 border-b-4 border-red-700 hover:border-red-500 rounded-xl" onClick={onResetAll}>
            Reset All
          </button>
          <button className="bg-amber-500 hover:bg-amber-400 text-white font-bold py-2 px-4 border-b-4 border-amber-700 hover:border-amber-500 rounded-xl" onClick={onReset}>
            Reset Robot
          </button>
          <button className="bg-emerald-500 hover:bg-emerald-400 text-white font-bold py-2 px-4 border-b-4 border-emerald-700 hover:border-emerald-500 rounded-xl" onClick={compute}>
            Submit
          </button>
        </div>
        ...
        <div className="grid grid-cols-4 gap-x-2 gap-y-4 items-center">
          {obstacles.map((ob, index) => {
            return (
              <div
                key={ob}
                className="badge flex flex-row p-3 text-violet-600 bg-violet-50 rounded-xl text-xs md:text-sm h-max border-violet-400 border-4"
              >
                <div className="flex flex-col">
                  <div className="font-bold" > Obstacle #{index+1} </div>
                  <div><b>X:</b> {ob.x} </div>
                  <div><b>Y:</b> {ob.y}</div>
                  <div><b>D:</b> {DirectionToString[ob.d]}</div>
                </div>
                <div>
                  <svg
      ...
      <div className="flex flex-col items-center justify-center py-20">
        {path.length > 0 && (
          <div className="flex flex-row items-center text-center my-8 bg-white pt-4 pb-4 pl-4 pr-4 rounded-lg border-none">
            <button
              className="btn btn-square pt-2 pl-1 bg-blue-600 hover:bg-blue-800 border-none"
              disabled={page === 0}
              onClick={() => {
                setPage(page - 1);
              }}
            ...
            <span className="mx-5 text-black text-xl font-medium">
              Step {page + 1}
            </span>
            <span className="mx-5 text-black text-xl font-medium">{commands[page]}</span>
            <button
              className="btn btn-square pt-2 pl-2 bg-blue-600 hover:bg-blue-800 border-none"
        ...
        <table className="border border-collapse border-none border-black rounded-lg">
